    ...
    public class ReviewersController : ControllerBase
    {
        ...
        public IActionResult GetReviewersById(int reviewerId)
        {
            if (!_reviewerRepository.ReviewerExist(reviewerId))
                return NotFound();
            var reviewer = _mapper.Map<ReviewerDto>(_reviewerRepository.GrtReviewerById(reviewerId));
            if (!ModelState.IsValid)
                return BadRequest(ModelState);
            return Ok(reviewer);
        }
        ...
        public IActionResult CreateReviewer([FromBody] ReviewerDto reviewerCreate)
        {
            ...
            if (!_reviewerRepository.CreateReiewer(reviewerMap))
            {
        ...
        [HttpDelete("reveiwerId")]
        [ProducesResponseType(400)]
        [ProducesResponseType(204)]
        [ProducesResponseType(404)]
        public IActionResult DeleteReveiwer(int reveiwerId)
        {
            if(!_reviewerRepository.ReviewerExist(reveiwerId)) return NotFound();
            
            var deletedReviewer = _reviewerRepository.GrtReviewerById(reveiwerId);
